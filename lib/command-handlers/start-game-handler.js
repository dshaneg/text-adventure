...
class StartGameHandler {
  ...
  handle() {
    ...
    bus.commandChannel.publish(new AddInventoryCommand(this.itemRepository.startSet));

    for (const startItem of this.itemRepository.startSet) {
      if (startItem.equip) {
        bus.commandChannel.publish(new EquipItemCommand(startItem.item));
      }
    }
    ...
    bus.commandChannel.publish(new TeleportCommand(this.mapNodeRepository.entryNode.id));
  }
}

...
class TextEngine {
  ...
  handleInput(input) {
    for (const parser of this.commandParsers) {
      ...
      if (command) {
        bus.commandChannel.publish(command);
        return;
      }
    }
      ...
      switch (envelope.topic) {
        ...
        case 'player.inventory.list-requested':
          ...
          for (const stack of data.items) {
            ...
            const suffix = stack.item.suffix ? ` ${stack.item.suffix}` : '';
            const name = `${stack.item.name}${suffix}`;
            console.log(style.normal(`${name} ${count}`));
          }

...
class TextEngine {
  ...
  start() {
    ...
    this.hookPreStartHandlers(this._game);
    ...
    this.hookPostStartHandlers(this._game);
  }

  hookPostStartHandlers(game) {
    // this event happens multiple times to initialize inventory. don't want the notifications during start
    game.on('inventory-added', (event) => {
      console.log(`You add ${pluralize(event.item.name, event.count, true)} to your pack.`);
      this._rl.prompt();
    });
  }

  hookPreStartHandlers(game) {
    ...
    game.on('inventory-requested', (event) => {
      if (!event.items.length) {
        console.log(style.gamemaster('Inventory is empty.'));
        return;
      }

      console.log(style.gamemaster('You are carrying the following items:'));
    ...
    game.on('game-stopped', () => {
      console.log((style.gamemaster('See you next time.')));
      this._rl.close();
    });

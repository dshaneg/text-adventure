...
class GameEngine {
  ...
  initializeGameDefinition() {
    ...
    def.banner = this._gameDefinitionRaw.banner.join('\n');
    def.opening = this._gameDefinitionRaw.opening.join('\n');
    def.help = this._gameDefinitionRaw.help.join('\n');

    def.entryNode = this.map.get(this._mapDefinitionRaw.start);

...
class Inventory {
    ...
    bus.eventChannel.subscribe({
      ...
      callback(data) {
        for (const delta of data.deltas) {
          self.add(delta.item, delta.count);
        }
      }
    });
  }
  ...
  subscribeQueries() {
    bus.queryChannel.subscribe('player.inventory', (data, envelope) => {
      envelope.reply(null, this.getAll());
    });
  }
  ...
  getEquipped() {
    return Array.from(this._equippedSet.values());
  }

  getAll() {
    const items = Array.from(this._map.values());
    return items;
  }

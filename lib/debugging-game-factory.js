...
class DebuggingGameFactory {
  ...
  createGame() {
    ...
    game.on(inventoryAdded, event =>
      console.log(style.debug(`event: ${inventoryAdded} ${event.count} (${event.item.id}) ${event.item.name}`))
    );

    const inventoryRemoved = 'inventory-removed';
    game.on(inventoryRemoved, event =>
      console.log(style.debug(`event: ${inventoryRemoved} ${event.count} (${event.item.id}) ${event.item.name}`))
    );

    const itemEquipped = 'item-equipped';
    game.on(itemEquipped, event =>
      console.log(style.debug(`event: ${itemEquipped} - (${event.item.id}) ${event.item.name}`))
    );

    const itemUnequipped = 'item-unequipped';
    game.on(itemUnequipped, event =>
      console.log(style.debug(`event: ${itemUnequipped} - (${event.item.id}) ${event.item.name}`))
    );
